...
const EdgeMarker = memo(
  ({ marker, delay, onHover }: { marker: (typeof regionMarkers)[0]; delay: number; onHover: (marker: (typeof regionMarkers)[0] | null) => void }) => {
    const color = "#0070f3"; // Bright Azure Blue
    ...
    return (
      <Marker coordinates={marker.coordinates}>
        ...
        >
          ...
          {/* Inner Solid core */}
          <circle cx={0} cy={0} r={5} fill={color} stroke="var(--ds-background-100)" strokeWidth={1.5} />
          {/* White centerpiece dot */}
...
export default function DottedMap({ width = 1000, height = 560 }: DottedMapProps) {
  ...
  const [liveMarkers, setLiveMarkers] = useState(regionMarkers);

  const fetchState = async () => {
    try {
      const res = await fetch("http://localhost:9000/state");
      const data = await res.json();
      if (data && data.markets) {
        const lookup: Record<string, [number, number]> = {
          "NZWN": [174.805, -41.327],
          "NYC_KLGA": [-73.874, 40.776],
          "SEA_KSEA": [-122.309, 47.449],
          "TOKYO_RJTT": [139.78, 35.549]
        };
        const mapped = Object.entries(data.markets).map(([name, m]: any) => ({
          id: name,
          name: m.market_name,
          coordinates: lookup[name] || [139.78, 35.54]
        }));
  ...
  useMemo(() => {
    fetchState();
    const int = setInterval(fetchState, 2000);
